...
model = dict(
    ...
    decode_head_s=dict(
        ...
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.25, 1.75, 1.0, 1.0])),
    decode_head_t=dict(
        type='SegformerHead',
        in_channels=[64, 128, 320, 576],
        in_index=[0, 1, 2, 3],
...
work_dir = './experiments/segformerb5/results_R2S/'
...
data = dict(
    ...
    train=dict(
        ...
        ann_dir='2021LoveDA_RuralforGF-2_5CLS/Rural_train/masks_png_4CLS', 
        split='2021LoveDA_RuralforGF-2_5CLS/TrainRural.txt',
        B_img_dir = 'Sentinel2/train/img_dir',
        B_split = 'Sentinel2/train.txt'),
    # target domain for validation
    val=dict(
        img_dir='Sentinel2/val/img_dir',
        ann_dir='Sentinel2/val/ann_dir_4CLS',
        split='Sentinel2/val.txt'),
    test=dict(
        img_dir='Sentinel2/val/img_dir',
        ann_dir='Sentinel2/val/ann_dir_4CLS',
        split='Sentinel2/val.txt'))
