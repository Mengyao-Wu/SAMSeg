_base_ = [
    '../../../configs/_base_/datasets/PC_daseg.py', '../../../configs/_base_/default_runtime.py',
]
# model settings
...
model = dict(
    ...
    decode_head_s=dict(
        ...
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.0, 1.5, 1.5])),
    decode_head_t=dict(
        type='SegformerHead',
        in_channels=[64, 128, 320, 576],
        in_index=[0, 1, 2, 3],
    ...
    cross_EMA = dict(
        ...
        decode_head_EMA=dict(
            ...
            align_corners=False,
            loss_decode=dict(
                type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.0, 1.25, 1.25]))
    ),
    # model training and testing settings
    train_cfg=dict(),
    test_cfg=dict(mode='slide', crop_size=(1024, 1024), stride=(768, 768)))

work_dir = './experiments/segformerb5/results_P2C/'
