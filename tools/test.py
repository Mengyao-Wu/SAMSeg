...
def parse_args():
    ...
    parser.add_argument(
        '--auto-show', default=False,action='store_true', help='directory where painted images will be saved')
    parser.add_argument(
        '--gpu-collect',
        action='store_true',
...
def main():
    ...
    if args.auto_show:
        args.show=True
        args.show_dir='.'.join(args.checkpoint.split('.')[:-1])
    if '+' in args.eval:
        args.eval = args.eval.split('+')
    # print(args.eval)
    # print(args.eval.split('+'))
    # raise None
